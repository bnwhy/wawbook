...
6. **Printing**: Final book printing is handled externally in InDesign (not generated by the system)
...
- **jsPDF**: PDF generation utilities in `utils/pdfGenerator.ts` (for admin/preview purposes only, not for final printing)
