...
const { parseIdmlBuffer } = require('./dist/index.cjs');

async function testChateauFix() {
  console.log('=== TEST: Correction des styles de caractères IDML ===\n');
  
  const idmlPath = path.join(__dirname, 'Le château 5.idml');
  ...
  console.log(`  - color: #6f1d76 (du CharacterStyle via fillColor)`);
  console.log(`  - strokeColor: #801a76 (du CharacterStyle)`);
